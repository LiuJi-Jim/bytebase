<template>
  ...
  >
    ...
    <div
      ref="treeContainer"
      class="databases-tree--tree flex-1 overflow-y-auto"
      :data-height="treeHeight"
    >
      <n-tree
        block-line
        :data="treeData"
...
const { height: treeHeight } = useElementSize(treeContainer);
...
const loadSubTree = async (option: TreeOption): Promise<void> => {
  ...
  if (item.type === "database") {
    ...
    item.children = tableList.map((table) =>
      generateTableItem(connectionTreeStore.mapAtom(table, "table", item.id))
    );
